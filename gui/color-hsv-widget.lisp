...
(defun draw-hsv-gradient-wheel (painter color size)
  (let ((radius (floor (/ size 2)))
    ...
    ;; Clear inner
    (with-objects ((brush (#_new QBrush (cached-color 'black))))
    ...
    (with-objects ((pen (#_new QPen)))
      (#_setWidthF pen 5.0)
      (#_setColor pen (cached-color 'white))
      (#_setPen painter pen)
      (let ((deg (- (/ (* (#_hsvHue color) PI) 180))))
        (#_drawLine painter
...
(defun draw-hsv-gradient-triangle (painter color size)
  (let ((radius (floor (/ size 2)))
        (wheel-width (floor (/ size 15))))
...
(defmethod update-color-triangle ((widget color-hsv-widget))
  (when (< 0 (size widget))
    ...
    (let* ((color (color widget))
           (size (size widget)))
      (with-objects ((pixmap (make-gl-pixbuf size size)))
        (with-painter (painter pixmap)
          (#_makeCurrent pixmap)
          ...
          (draw-hsv-gradient-wheel painter color size)
          (draw-hsv-gradient-triangle painter color size)
