...
(defclass curve-dialog ()
  ((%picker :accessor picker)
   ...
   (%button-refresh :accessor button-refresh))
  (:metaclass qt-class)
...
(defmethod initialize-instance :after ((dialog curve-dialog) &key)
  ...
  (let ((layout (#_new QGridLayout))
    ...
    (setf (picker dialog) picker
          ...
          (button-refresh dialog) button-refresh)
    (#_setValue distance 2.0)
    (#_setMinimum distance 0.1)
    (#_setMaximum distance 200)
    (loop for class in (c2mop:class-direct-subclasses (find-class 'curve))
          for i from 0
          do (#_addItem picker (string-downcase (class-name class)))
             (when (eql (class-name class) *curve-type*)
               (#_setCurrentIndex picker i)))
    (#_addWidget layout picker 0 0 1 4)
    (#_addWidget layout distance 0 4 1 1)
    (#_addWidget layout button-refresh 0 5 1 1)
    (#_addWidget layout canvas 1 0 1 6)
    (#_addWidget layout button-ok 2 0 1 3)
    (#_addWidget layout button-cancel 2 3 1 3)
    ...
    (connect button-refresh "clicked()" dialog "refresh()")
    (select-curve dialog *curve-type*)))

(defmethod select-curve ((dialog curve-dialog) name)
  (let ((class-name (find-symbol (string-upcase name) "PARASOL")))
    (when class-name
      (refresh dialog :name class-name))))
...
(defmethod paint-event ((canvas curve-dialog-canvas) event)
  (when (curve canvas)
    (with-painter (painter canvas)
      (with-objects ((black (#_new QColor 0 0 0))
...
(defmethod refresh ((dialog curve-dialog) &key name)
  (let ((curve (make-curve (or name (class-name (class-of (curve (canvas dialog))))))))
    (setf (point-distance curve) (#_value (distance dialog)))
    (record-point curve 20  150 0 0 0)
    (record-point curve 100 220 0 0 0.5)
    (record-point curve 180  50 0 0 1.0)
    (record-point curve 250  50 0 0 0.9)
