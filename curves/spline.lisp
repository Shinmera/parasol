            ...
            (let ((dx (- (aref distances khigh)
              ...
              (if (= 0 dx)
                  (/ (+ (aref x klow) (aref x khigh))
                     2)
                  (let ((ai (/ (- (aref distances khigh) p)
                               dx))
                        (bi (/ (- p (aref distances klow))
                               dx)))
                    (+ (* ai (aref x klow))
                       (* bi (aref x khigh))
                       (/ (* (+ (* (- (expt ai 3) ai)
      ...
      (when (< 1 len)
        (loop for data across (data spline)
              ...
              do (ensure-length inter point-amount)
                 (loop for j from from below point-amount
                       do (setf (aref inter j)
                                (interpolate spline data spline-data (* j point-distance)))))))))
